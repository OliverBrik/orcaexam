...
if ( 'POST' === $_SERVER['REQUEST_METHOD'] && isset( $_POST['orca_blog_action'] ) ) {
    ...
    if ( 'create_post' === $action ) {
        if ( ! is_user_logged_in() || ! current_user_can( 'publish_posts' ) ) {
            $error = orca_text('Log ind med en konto, der kan udgive indlæg.', 'Please sign in with an account that can publish posts.');
    ...
    if ( 'comment' === $action ) {
        ...
        if ( ! isset( $_POST['orca_blog_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['orca_blog_nonce'] ) ), 'orca_comment_' . $post_id ) || ! comments_open( $post_id ) ) {
            ...
            if ( '' === $comment || ( ! is_user_logged_in() && ( '' === $name || ! is_email( $email ) ) ) ) {
                $error = orca_text('Indtast en kommentar, et navn og en gyldig e-mailadresse.', 'Please enter a comment, name, and valid email address.');
            } else {
...
?>
<?php get_header(); ?>
    <?php /* Legacy inline blog styling removed; styles are provided by style.css.
    ...
    */ ?>
<section class="orca-blog-hero">
    <div class="orca-blog-hero__inner">
        <div class="orca-blog-hero__content">
            <p class="orca-contact__kicker"><?php echo esc_html( orca_text('Orca fællesskab', 'Orca community') ); ?></p>
            <h1><?php echo esc_html( get_the_title() ?: orca_text('Fællesskabsblog', 'Community Blog') ); ?></h1>
            <p><?php echo esc_html( orca_text('Læs nyt, få inspiration, og del erfaringer med andre i Orca-fællesskabet.', 'Read updates, find inspiration, and share experiences with the Orca community.') ); ?></p>
        </div>
    </div>
</section>
...
<main class="orca-blog">
    ...
    <details class="orca-filter">
        <summary><?php echo esc_html( orca_text('Filtrer', 'Filter') ); ?></summary>
        <form method="get" action="<?php echo esc_url( $page_url ); ?>">
            ...
            <div>
                ...
                <div class="orca-sort-options">
                    <button class="<?php echo in_array( $filter_sort, array( 'most-liked', 'least-liked' ), true ) ? 'orca-active-sort' : ''; ?>" type="submit" name="blog_sort" value="<?php echo 'most-liked' === $filter_sort ? 'least-liked' : 'most-liked'; ?>"><?php echo esc_html( orca_text('Likes', 'Likes') ); ?><?php if ( in_array( $filter_sort, array( 'most-liked', 'least-liked' ), true ) ) : ?> <?php echo 'most-liked' === $filter_sort ? '↓' : '↑'; ?><?php endif; ?></button>
                    <button class="<?php echo in_array( $filter_sort, array( 'most-commented', 'least-commented' ), true ) ? 'orca-active-sort' : ''; ?>" type="submit" name="blog_sort" value="<?php echo 'most-commented' === $filter_sort ? 'least-commented' : 'most-commented'; ?>"><?php echo esc_html( orca_text('Kommentarer', 'Comments') ); ?><?php if ( in_array( $filter_sort, array( 'most-commented', 'least-commented' ), true ) ) : ?> <?php echo 'most-commented' === $filter_sort ? '↓' : '↑'; ?><?php endif; ?></button>
                    <button class="<?php echo in_array( $filter_sort, array( 'newest', 'oldest' ), true ) ? 'orca-active-sort' : ''; ?>" type="submit" name="blog_sort" value="<?php echo 'newest' === $filter_sort ? 'oldest' : 'newest'; ?>"><?php echo esc_html( orca_text('Dato', 'Date') ); ?><?php if ( in_array( $filter_sort, array( 'newest', 'oldest' ), true ) ) : ?> <?php echo 'newest' === $filter_sort ? '↓' : '↑'; ?><?php endif; ?></button>
                </div>
            </div>
            <button type="submit" name="blog_sort" value="<?php echo esc_attr( $filter_sort ); ?>"><?php echo esc_html( orca_text('Anvend kategori', 'Apply category') ); ?></button>
            ...
            <article class="orca-card" id="post-<?php echo esc_attr( $post_id ); ?>">
                ...
                <div class="orca-actions">
                    <form method="post">
                        ...
                        <button class="orca-like-button" type="submit" <?php disabled( isset( $_COOKIE['orca_liked_' . $post_id] ) ); ?>>♥ <?php echo esc_html( orca_text('Like', 'Like') ); ?></button>
                    </form>
                    <span><?php echo esc_html( $likes ); ?> <?php echo esc_html( 1 === $likes ? orca_text('like', 'like') : orca_text('likes', 'likes') ); ?></span>
                </div>
                <details class="orca-comments" <?php echo $error && 'comment' === ( $_POST['orca_blog_action'] ?? '' ) && $post_id === absint( $_POST['post_id'] ?? 0 ) ? 'open' : ''; ?>>
                    ...
                    <?php if ( comments_open( $post_id ) ) : ?>
                        <form class="orca-form" method="post">
                            ...
                            <?php if ( ! is_user_logged_in() ) : ?><label><?php echo esc_html( orca_text('Navn', 'Name') ); ?><input name="comment_author" required></label><label><?php echo esc_html( orca_text('E-mail', 'Email') ); ?><input type="email" name="comment_email" required></label><?php endif; ?>
